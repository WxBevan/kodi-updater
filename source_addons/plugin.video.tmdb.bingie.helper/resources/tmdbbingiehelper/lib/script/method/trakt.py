...
from tmdbbingiehelper.lib.script.method.decorators import is_in_kwargs, get_tmdb_id
...
def like_list(like_list=None, user_slug=None, delete=False, **kwargs):
    ...
    if not delete:
        return
    from tmdbbingiehelper.lib.script.method.kodi_utils import container_refresh
    container_refresh()
...
def sort_list(**kwargs):
    from xbmcgui import Dialog
    from tmdbbingiehelper.lib.addon.plugin import executebuiltin, format_folderpath, encode_url
    from tmdbbingiehelper.lib.api.trakt.sorting import get_sort_methods
...
def refresh_trakt_sync(**kwargs):
    from xbmcgui import Dialog
    from tmdbbingiehelper.lib.addon.plugin import get_localized, executebuiltin
